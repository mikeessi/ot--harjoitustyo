...
class GameLoop:
    ...
    def handle_endpiles(self, mouse_pos):
        ...
        if endpile is not None:
            self.no_hits = False
            if self.currently_dragging is True:
                if self.endpile_list[endpile].check_move(self.dragged_card.card) is True:
    ...
    def handle_tableaus(self, mouse_pos):
        ...
        if tableau_num is not None:
            ...
            if self.currently_dragging is False:
                ...
                if self.tableau_list[tableau_num].check_move(self.dragged_card) is True:
